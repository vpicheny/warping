...
%  \gaeladd{A quoi servent les couleurs sur ces 4 graphiques ? Si ça represente la valeur de l'output, ne faudrait-il pas le preciser ?}
\end{figure}
...
On all cases, a Gaussian warping (exponential with $d=2$) is applied, with range parameter $\theta=0.3$.
 ...
 \begin{figure}[!ht]
 \centering
 \includegraphics[trim=8mm 5mm 15mm 10mm, clip, width=.4\textwidth]{defAND.pdf}
  \includegraphics[trim=8mm 5mm 15mm 10mm, clip, width=.4\textwidth]{def3DOR2s.pdf}\\
 \includegraphics[trim=8mm 5mm 15mm 10mm, clip, width=.4\textwidth]{def3Dlin.pdf}
 \includegraphics[trim=8mm 5mm 15mm 10mm, clip, width=.4\textwidth]{def3Dcirc2.pdf}\\
  \includegraphics[trim=8mm 5mm 15mm 10mm, clip, width=.4\textwidth]{def3Dedge.pdf}
 \caption{Four deformations of the unit cube under different invariances: AND (top left), OR (top right), LINEAR (middle left), CIRCULAR (middle right). 
 The bottom figure shows the orginal space. 
%  Left: original space (with the critical edge highlighted). Centered: warping for $c_1=0$ AND $c_2=1$.
%  Right: warping for $x_1 = x_2$. Warping with circular conditions. Right: distorted space.
%  Warping for $c_1=4$ (left face of the cube) OR $c_2=1$ (front face).
...
% \gaeladd{Notation a expliciter ici.}
...
\gaeladd{ne faudrait-il pas developper un peu cet argument qui permet d'indiquer la plus-value de l'approche par rapport à ce qui existe deja ?}
% 
