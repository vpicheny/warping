...
% This disease is caused by a virus which induces numerous damages on \textit{Prunus} trees.
...
% 
% We show that the final result of the optimization is not impacted by the use of the warping. 
% However, it impacts the optimization speed. Indeed, the optimization process with warping allowed finding the best result faster than without. 
% Such warping process could be used for various optimization issues presenting invariances in order to reduce the calculation time.
...
In this study, we analyse a simulation model of sharka disease spread and management. This disease, caused by a virus transmitted by aphids, is one of the most damaging diseases of stone fruit trees belonging to the genus \textit{Prunus} (e.g. peach, apricot and plum) \cite{cambra2006plum,rimbaud2015sharka}.
...
methodology for sequential design~\cite{box1987empirical}, where the basic idea is to replace an expensive \gaeladd{Preciser ici que c'est l'evaluation qui est couteuse (costly, je pense).} function by a cheap-to-evaluate surrogate one.
...
\end{eqnarray}
\gaeladd{J'ai un doute sur l'equation 1. k(x) etant defini comme la transpose du vecteur des k(x,xi), on transpose la transposee (pour rien, en qq sorte). N'est-il pas plus simple de définir k(x) comme un vecteur non transpose ? Idem pour l'equation 2. }\\
\gaeladd{Dans l'equation 2, je note que k(x') n'est pas transpose, donc si on redefinit k(x), il faudrait avoir "k(x') puissance T".}\\
...
both kriging models are updated. One particular characteristic of this strategy is that it does not allow repetitions \gaeladd{J'aurais dit le contraire, ce qui indique que j'ai mal compris au moins le terme repetitions (=replicates ??). Sinon, comment faire une moyenne sans repetitions ?} \coralieadd{Je comprends ici que "repetitions" correspond au terme "iteration" que j'emploi habituellement, cad aux nombres d'etapes dans l'optimisation. c'est bien ca ? il faudrait peut etre clarifier le terme}, which is desirable in our
case.
...
Here, we formalize the different situations \gaeladd{A-t-on ici l'ensemble des types d'invariance possibles ? Si oui, il faudrait peut etre en parler rapidement pour ne pas parler que de la definition 3 qui n'est pas dans l'illustration de la sharka} and generalize some cases.
...
\definition[Simple]{We call \textit{simple invariance} the following case: $y$ is invariant with respect to $\x_J$ ($J$ a subset of $\{1, \ldots, d\} \setminus i$) if $x_i = c_i$ ($i \in \{1, \ldots, d\}$):
\begin{eqnarray*}
 y(c_i, \x_J, \x_{-iJ}) = y(c_i, \x_J', \x_{-iJ}), \qquad \forall \x_J, \x_J' \in \Xset_J, \x_{-iJ} \in \Xset_{-iJ}.
\end{eqnarray*}
}
...
\definition[Or]{We call \textit{``or'' invariance} the following case: $y$ is invariant with respect to $\x_J$ ($J$ a subset of $\{1, \ldots, d\} \setminus I$) if 
there exists at least one $i \in I$ such that $x_i = c_i$ ($I$ a subset of $\{1, \ldots, d\} \setminus J$):
\begin{eqnarray*}
 y(c_i, \x_{I \setminus i}, \x_J, \x_{-IJ}) = y(c_i, \x_{I \setminus i}, \x_J', \x_{-IJ}), \qquad \forall \x_J, \x_J' \in \Xset_J, \x_{I \setminus i}, \in \Xset_{I \setminus i}.
\end{eqnarray*}
}
...
\definition[Linear]{We call \textit{linear invariance} the following case:
$y$ is invariant with respect to $\x_J$ ($J$ a subset of $\{1, \ldots, d\} \setminus I$) 
if $\mathbf{A} \x_I = \mathbf{b}$, with $I$ a subset of $\{1, \ldots, d\} \setminus J$),
 $\mathbf{A}$ a matrix of size $p \times \card(I)$ and $\mathbf{b}$ a vector of size $p$:
\begin{eqnarray*}
 y(\x_{I}, \x_J) = y(\x_{I}, \x_J'), \qquad \forall \x_J, \x_J' \in \Xset_J, \text{ if } \mathbf{A} \x_I = \mathbf{b}.
\end{eqnarray*}
}
...
% 
% $y$ is invariant with respect to $\x_J$ ($J$ a subset of $\{1, \ldots, d\} \setminus I$) if 
% there exists at least one $i \in I$ such that $x_i = c_i$ ($I$ a subset of $\{1, \ldots, d\} \setminus J$)
% 
% % \paragraph{Simple invariance} The simplest case involves only a single variable, and can be stated as:
%  
% 
% 
...
which is in contradiction with the notion of local invariance. \gaeladd{Cette info ne devrait-elle pas etre amenee en amont pour justifier l'intéret de la question traitee ?}
...
\end{enumerate}
Condition 1 ensures that $\widetilde{\x_j} = \overline{\x_j}$ when $x_i=c_i$ (the J-th dimensions \gaeladd{Les J-emes dimensions ?} collapse).


\subsubsection{Warping for a simple invariance}
...
\centering
 ...
 \caption{Three deformations of a 2D space. The local invariance is at $x_1=0$, highlighted with larger lines.}\label{fig:3defsimple} \gaeladd{A quoi servent les couleurs sur ces 4 graphiques ? Si ça represente la valeur de l'output, ne faudrait-il pas le preciser ?}
\end{figure}
...
% cannot exist in 2D under the assumption $I \cap J = \emptyset$, since $\card(J) \geq 2$.}. 
...
Figure \ref{fig:def3Dedge} shows two deformations of the unit cubic space when $y$ is invariant w.r.t. $x_3$ when 1- $x_1=x_2=0$, and 2-$x_1 = x_2$. 
On both cases a Gaussian warping (exponential with $d=2$) is applied.
 \begin{figure}[!ht]
 \centering
 ...
 Right: warping for $x_1 = x_2$.\gaeladd{Penser a revoir les graduations avant soumission.Par ailleurs, pour faciliter la comparaison entre les 3 cubes, il me semble qu'il faudrait les presenter sous le meme angle.}
 %Note that the axis has been rotated to show the shape of the distorted space.
 }\label{fig:def3Dedge}
\end{figure}
...
However, we found in our numerical experiments that choosing the same correlation function for the GP and the warping, and fixing 
...
A possible solution is $\theta_A = \mathbf{A}^T \boldsymbol{\theta}_I$. \gaeladd{Notation a expliciter ici.}
...
In addition, we can visually observe that the warping step allow to improve the optimization speed on the Fig.\ref{fig:algococo10000} and Supplementary Fig.\ref{fig:algococo0}.
...
%\begin{figure}[!ht]
%	\centering
%	\includegraphics[trim = 1.1cm 20.8cm 5.8cm 3.3cm, clip, width=\textwidth]{Figures_Warping_resultats_courbes_regression_lineaire_warping_sanswarping.pdf}
...
% the parameter invariances. 
% These invariances correspond to the insensitivity of the model to a subset of parameters for some values of another subset of parameters. 
% Here, we take as an example the optimization problem of sharka control. 
...
% 
% Firstly, we presented how we took into account such invariances in the Bayesian optimization process with the warping. 
% Then, we applied this process to the sharka model in order to analyze the warping efficiency.
...
A particular advantage of input warping over other approaches is that it can be straightforwardly embedded in a BO algorithm. \gaeladd{ne faudrait-il pas developper un peu cet argument qui permet d'indiquer la plus-value de l'approche par rapport à ce qui existe deja ?}
% 
% In order to tackle invariances due to the management parameters, we adapted a Bayesian optimization algorithm by including a warping to the parameter space.
...
As future steps for this research, we could first embed learning the warping parameters together with the parameters of the GP covariance 
...
% We showed that the optimization results are not impacted by the use of the warping. 
% This means that if the optimization process includes enough iterations, the warping does not improve the result. Nevertheless, the optimization speed was impacted: 
% the optimization process with warping was faster than without. This shows that we could perform less iterations to obtain the same result, which means we could save time.
% 
% Thus, in order to reduce the calculation time, such warping approach could be used for various optimization issues presenting invariances. 
% For instance, we could use it to optimize all management parameters included in the spatio temporal sharka model developed by Rimbaud et al.
% \cite{rimbaud2018heuristic} 
% (here, we optimized only 10 out of 23 management parameters included in the model). 
% Nevertheless, it may be difficult to define the minimum number of iteration to perform to reach the optimal value. 
% However, the warping use cannot affect negatively the optimization performance. Indeed, if numerous iterations are performed, the warping use will probably be unnecessary. 
% Conversely, if the number of iterations is limited, the warping can allow obtaining better results.

%\section*{References}
\bibliographystyle{spbasic}
\bibliography{refs}
%>>>>>>> ab2d126e6e2bd64a6b30e0fa16427765853a333a
